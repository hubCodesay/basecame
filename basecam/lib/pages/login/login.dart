...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> onSignIn() async {
    ...
    try {
      ...
      debugPrint('SignIn result: user=${cred.user?.uid}');
      ...
      if (uid != null) {
        try {
          ...
          if (!doc.exists) {
            debugPrint('User doc for uid=$uid not found in users collection');
            // user signed in but no user document — offer to create
            _showUserNotFoundDialog(email);
            return;
          } else {
            debugPrint('User doc exists for uid=$uid');
          }
        } on FirebaseException catch (e) {
          debugPrint(
            'Error while fetching user doc for uid=$uid: ${e.code} ${e.message}',
          );
          // If Firestore is unavailable (client offline), allow the user to
          // proceed because authentication succeeded; inform them about
          // offline status and limited functionality.
          if (e.code == 'unavailable') {
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'No network: working offline. Some data may be unavailable.',
                  ),
                ),
              );
            }
            // proceed to app even if user document couldn't be read now
          } else {
            // For other Firestore errors, rethrow so upstream handling shows an error dialog
            rethrow;
          }
        }
      }
